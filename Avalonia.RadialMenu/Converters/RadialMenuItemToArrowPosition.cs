...
namespace Avalonia.RadialMenu.Converters
{
    internal class RadialMenuItemToArrowPosition : IMultiValueConverter
    {
        public object Convert(IList<object?> values, Type targetType, object? parameter, CultureInfo culture)
        {
            ...
            if (values.Count != 5)
            {
                throw new ArgumentException("RadialMenuItemToArrowPosition converter needs 5 values (double centerX, double centerY, double arrowWidth, double arrowHeight, double arrowRadius) !", "values");
            }
            if (parameter == null)
            {
